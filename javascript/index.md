...
在正常的js运转当中基本不会存在阻塞行为，因为所有的运行机制都是基于异步去实现的

### 事件队列

事件队列指的是在每次监听到浏览器的任务时会生成一个事件，然而这个事件未必会马上执行，而且存放在一个事件队列里面，只有当前一个内容处理完毕才会去加载后一个队列里面的信息，但是这些又是一种异步的行为，即如果不断的添加事件，也不会造成浏览器的卡顿，只会说每次执行的时间可能跟预想的会有一定的区别，但是迟早会执行到当前的任务
