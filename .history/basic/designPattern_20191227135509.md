...
模板方法模式就是提供这么一个方法，他通过提取公共内容完成从虚拟父类继承到实现子类的过程
...
我们重新定义了一个`Node类`，使他拥有父子节点的共同方法跟参数，然后将子类继承于父类，这就是模板方法模式，当然在某些特殊的情况下，我们会为`Node类`添加一个钩子，让我们能够在执行方法的时候，预先执行这个钩子方法

针对我们打印节点方法是一样的，那么这个打印节点就是我们的模板方法，通过这个一样的操作方式能够定义不同方法，这里面其实有点不太合适，因为scan方法实际上只做了一件打印的事情